...
export default function DualTimeline({ credentials = [] }) {
  ...
  return (
    <section className="relative w-full py-10 md:py-16 overflow-hidden">
      {/* MOBILE CONTENT FLOW (Hidden on Desktop) */}
      <div className="block md:hidden space-y-16 px-4">
        {/* Education Mobile Block */}
        <div className="space-y-6">
          <div className="border-b border-zinc-900 pb-4">
            <span className="text-[10px] uppercase tracking-[0.4em] text-zinc-500 font-bold block mb-1">
              2012 - 2027
            </span>
          ...
          <div className="flex flex-col gap-6">
            {education.map((edu, i) => (
              <Card key={`edu-mob-${edu.id || i}`} variant="education">
                <div className="flex items-start justify-between mb-6">
                  ...
                  {edu.isOngoing && (
                    <span className="px-3 py-1 rounded-full border border-fuchsia-500/20 bg-fuchsia-500/10 text-[9px] uppercase tracking-wider font-bold text-fuchsia-300 shrink-0">
                      Ongoing
                    </span>
                  )}
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-6">
            {credentials.map((cred, i) => (
              <Card key={`cred-mob-${cred.id || i}`} variant="credential">
                ...
                <div className="mb-5">
                  ...
                  <p className="text-xs text-zinc-500 mt-1">{cred.date}</p>
                </div>
                <div className="border-t border-zinc-800/60 pt-4">
                  <p className="text-xs leading-6 text-zinc-300">
                    {cred.description}
                  ...
                  {cred.url && (
                    <div className="mt-4">
                      <a
                        ...
                        className="inline-flex items-center gap-2 text-xs text-purple-400 hover:text-white transition-colors"
                      >
                        Verify Credential
      ...
      {/* DESKTOP TIMELINE FLOW (Hidden on Mobile viewports) */}
      <div className="hidden md:block">
        {/* TOP HEADER SECTION */}
        <div className="max-w-[1000px] mx-auto px-6 mb-20 flex flex-row justify-between items-start gap-10">
          {/* Education Header */}
          ...
          <div className="group/title cursor-default text-right flex flex-col items-end">
            <span className="text-[11px] uppercase tracking-[0.4em] text-zinc-500 font-bold block mb-1">
              2023 - Present
            </span>
            <h2 className="text-5xl font-black italic uppercase text-white tracking-tighter">
        ...
        {/* Timeline spine */}
        <div className="absolute left-1/2 top-52 bottom-0 w-px bg-gradient-to-b from-fuchsia-500/40 via-purple-500/20 to-transparent" />
        ...
        <div className="flex flex-col gap-12">
          {education.map((edu, i) => {
            ...
            return (
              <div
                key={edu.id || i}
                className="relative flex flex-row justify-between items-stretch gap-8"
              >
                {/* EDUCATION CARD */}
                ...
                {/* TIMELINE DOT */}
                <div className="flex absolute left-1/2 top-12 -translate-x-1/2 z-20">
...
// Inline fallback helper to ensure standard styling if custom wrappers are missing
function ProjectIconWrapper({ children }) {
  return <>{children}</>;
}
